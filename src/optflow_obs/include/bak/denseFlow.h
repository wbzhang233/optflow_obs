...
PtsFlowFoe getSparseRansacFOE1(vector<Point2f> Pts,vector<Point2f> flows,int iterTimes=200,double epislon=10.0);
...
// 新增:获取区块的点集合
// 获取区块上的四个顶点处光流构成的A和b
//void getBlockPts(Mat flow,Size blockSize,vector<Point2f> &A,vector<float> &b)
//{
//
//
//}
...
/// 1-5 RANSAC 最小二乘
Point2f getDenseRansacFOE(Mat flow,int iterTimes=100,double epsilon=10.0){
    assert(flow.channels()==2);
...
//    cout<<"init_size:"<<initFlowPts.pts.size()<<endl;
    ...
    // 1-稀疏光流RANSAC估计方法
    PtsFlowFoe  res = getSparseRansacFOE1(initFlowPts.pts,initFlowPts.flows,iterTimes,epsilon);
...
PtsFlowFoe getSparseRansacFOE1(vector<Point2f> Pts,vector<Point2f> flows,int iterTimes,double epislon){
    PtsFlowFoe res;
...
/// 1-航向角
//float thetas[] = {-40,-30,-20,-10,0,10,20,30,40};
float nav_thetas[] = {-35,-25,-15,-5,0,5,15,25,35};
Point2f getAmuziVel(int amuzi,float mag = 2.0){
    Point2f vel;
    vel.y = mag;
    amuzi -= 4;
    vel.x = vel.y*tan(nav_thetas[amuzi]*M_PI/180);
...
void plotVbar(Mat &img,int amuzi,int stroop=8){
    ...
    for(auto ele:stroops){
        if(j==amuzi || j==amuzi+1){
            cv::line(img,Point2d(ele,0),Point2d(ele,height),Scalar(0,255,0),2,LINE_8);
        }else{
            cv::line(img,Point2d(ele,0),Point2d(ele,height),Scalar(120,0,0),1,LINE_8);
        }
        j++;
    }
    putText(img,"ENTRY",Point(cvRound((2*amuzi+1)*width/(2*stroop)), cvRound(height/2)),
            FONT_HERSHEY_PLAIN,1.0,Scalar(0,0,255),1,LINE_AA,false);
}
...
int balanceTTC(Mat ttcResult,int stroop=8)
{
    ...
    for(int i=0;i<stroop;++i){
        ...
        for(int col =low; col<high; ++col){
            for(int row=0; row<height; ++row){
                stroopTTC[i] += ttcResult.at<uchar >(row,col)/stroop_area;
            }
        }
    }
...
const vector<float> steerings =  {-35,-25,-15,-5,0,5,15,25,35};
vector<float> nav_sgn = {-1,-1,-1,-1,1,1,1,1};
// 求ttc的平均值
float getMean(vector<float> ttc){
    if(ttc.size()==0) return 0.0;
    float res = 0.0;
    for(auto ele:ttc){
        res += ele/ttc.size();
    }
...
Point2f SteeringAPF(Point2f foe,Mat ttcResult){
    ...
    if(foe_ind>=0 && foe_ind<=stroop-1){
         usc += k0*nav_sgn[foe_ind]*exp(-cpsi*abs(steerings[foe_ind])/35.0)*exp(-cd*abs(foe_ttc)); // 角度越大,产生的作用越大;越近,到达时间越小,产生的作用越大
    }
    for(int i=0;i<3;i++){
        usc -= k0*nav_sgn[i]*exp(-cpsi*abs(steerings[i])/35.0)*exp(-cd*abs(ttc[i]));
    }
...
/// 人工势场法2
Point3f APFDirection(Point2f foe, Mat ttcResult){
    Point3f res;
