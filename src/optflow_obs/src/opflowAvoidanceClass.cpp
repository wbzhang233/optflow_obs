...
const cv::Mat K = (Mat_<float>(3,3)<<
                                   320.0, 0.0, 160.0,0.0, 240.0, 120.0,0.0, 0.0, 1.0); //相机的本质矩阵
...
//             1-计算FOE
            ...
            PtsFlowFoe ransacRes = getDenseRansacFOE(disFlow,loss_final,10000,8.0);
            double foe_end =  getTickCount();
            cout<<"***************************"<<endl;
            cout<<"RANSAC:"<<(double) (foe_end - foe_start)/getTickFrequency()<<endl;
            cout<<"***************************"<<endl;
            Point2f rfoe = ransacRes.foe;

//            Point2f rfoe = getDenseRansacFOE(flow,loss_final,35);
//            cout<<"FOE:"<<rfoe.x<<" "<<rfoe.y<<endl;
//            cout<<"images_size:"<<image.size()<<endl;
            ...
            cout<<"ransacRes_size:"<<ransacRes.pts.size()<<endl;
//            vector<Point2f> keypts1(ransacRes.pts);
//            vector<Point2f> keypts2;
//            for(int i=0;i<ransacRes.pts.size();++i){
//                Point2f nextPt = ransacRes.pts[i]+ransacRes.flows[i];
//                keypts2.push_back(nextPt);
//            }
//            if(ransacRes.pts.size()>10){
//                cv::Mat E = cv::findEssentialMat(keypts1, keypts2, K,RANSAC);
////            从本质矩阵中恢复R,t
//                cv::Mat R,t;
//                cv::recoverPose(E, keypts1, keypts2, K, R, t);
//                // 输出R,T
//                cout<<"------ R Matrix ------"<<endl;
...
////            Eigen::Matrix3d rotation_matrix = mat2Matrix3d(R);
//                Eigen::Matrix3d rotation_matrix;
//                rotation_matrix<<R00,R01,R02,R10,R11,R12,R20,R21,R22;
//                Eigen::AngleAxisd rotation_vector(rotation_matrix);
//                Eigen::Vector3d eulerAngle = rotation_matrix.eulerAngles(2,1,0); // yaw pitch roll
//                // 输出欧拉角
//                cout<<"------ eulerAngle ------"<<endl;
//                for (int k = 0; k < 3; ++k) {
//                    cout<<eulerAngle(k)*(180/M_PI)<<"\t";
//                    if(abs(eulerAngle(k))> 10 ) notRotated = false;
//                }
//            }
