//
// Created by wbzhang233 on 2020/3/26.
//
...
int main(int argc, char **argv)
{
    ...
    {
        char image_name[30];
        sprintf(image_name,"/frame%04d.jpg",idx);
        string filepath = folderpath + image_name;
        frame = imread( filepath);
        if (frame.empty())
            break;
        // 调整大小
        cv::resize(frame,frame,cv::Size(0.5*frame.cols,0.5*frame.rows),0,0,cv::INTER_LINEAR);
...
//            imshow("munsell_flow", munsell_flow);
            ...
            //Point2f foe = getDenseFOE(flow,5); //使用正方形格子
            ...
            circle(plotFOE,foe,6,Scalar(255,0,255),2,8);//品红
//
//            // 1-4-2 迭代计算FOE
